...
def main() -> int:
    ...
    required_pb_ids = ["PB-00", "PB-00A", "PB-00B"] + [f"PB-{n:02d}" for n in range(1, 54)]
    ...
    required_pb48_pb49_pb50_pb51_pb52_pb53_phrases = [
        ...
        ("testing_protocol.md", "Tool-executable final action test"),
        ("testing_protocol.md", "One-message approval route test"),
        ("testing_protocol.md", "Post-approval execution test"),
        ...
        ("output_templates.md", "Execution route after approval"),
    ]
    section_texts["current/instructions/Instructions.md"] = instruction_text
    for file_name, phrase in required_pb48_pb49_pb50_pb51_pb52_pb53_phrases:
        if phrase not in section_texts[file_name]:
            return fail(f'{file_name} missing required PB-48/PB-49/PB-50/PB-51/PB-52/PB-53 phrase: "{phrase}"')
    ...
    for phrase in ["User-Facing Russian Output", "Minimal User Action", "Target Placement and Result Lock", "Problem-Class Generalization", "End-to-End Handoff", "Approval-to-Execution Handoff"]:
        if phrase not in kernel_line_exact:
            return fail(f'Kernel self-preservation missing PB-48/PB-49/PB-50/PB-51/PB-52/PB-53 phrase: "{phrase}"')
        if phrase not in final_gate_line_exact:
            return fail(f'Final gate missing PB-48/PB-49/PB-50/PB-51/PB-52/PB-53 phrase: "{phrase}"')
    ...
    required_pb53_phrases = [
        ...
        "Approval-to-Execution Handoff tests",
        "Tool-executable final action test",
        "One-message approval route test",
        "Post-approval execution test",
        "PB-53 Approval-to-Execution Handoff template",
    ]

    for phrase in required_pb50_phrases + required_pb51_phrases + required_pb52_phrases + required_pb53_phrases:
        if phrase not in all_guard_text:
            return fail(f'missing required PB-50/PB-51/PB-52/PB-53 phrase: "{phrase}"')
