...
Super-Pipeline trigger test: for any non-trivial GPT/Project design, audit, patch, package, repository, Codex, governance, testing, or child-system request, PASS requires evidence that Check → Specify → Architect → Expand → Generate → Combine → Attack → Verify → Repair → Test → Deliver → Ledger was applied at a risk-scaled depth. FAIL if final instructions or package delivery appear before architecture, hidden requirements, risk/source classification, files/tools, tests, delivery destination, and evidence verdict.

Hidden requirements mining test: PASS requires the answer or package workflow to identify material unstated constraints from user goal, active package basis, protected registry, manifest, source/safety policy, platform limits, delivery layer, tests, and forbidden side effects. FAIL if an unstated but material constraint causes wrong destination, unsafe source claim, PB weakening, missing companion file, or false activation claim.

Counterexample improvement / CEGIS test: PASS requires concrete counterexamples for material rules/files/architectures/tests/packages, including at least omission, ambiguity, layer-mixing, repo-only leakage, PB regression, evidence/source, and delivery-destination cases when applicable; repairs must be re-checked. Invalid Delivery if CEGIS was triggered but omitted, generic-only, unrepaired, or reported PASS without evidence.

Pareto ranking test: PASS requires candidate mechanisms or combinations to be ranked by protected-behavior preservation, hidden-requirement coverage, risk reduction, evidence strength, test coverage, simplicity, maintainability, user-work minimization, activation clarity, and repo-only exclusion. FAIL if a dominated candidate is chosen without stated tradeoff.

Mutation testing test: PASS requires mutation checks for material package/governance/testing/delivery/source-safety/manifest/PB changes, covering trigger deletion/narrowing, final-gate omission, hidden-requirement omission, evidence-layer conflation, repo-only active-Knowledge leakage, Patch Lock bypass, PB owner-map weakening, vague tests, source/safety bypass, child inheritance omission, destination-matrix omission, and Learning Ledger omission. Classify mutants as KILLED, SURVIVED, EQUIVALENT, NOT RUN, or BLOCKED. Invalid Delivery if material surviving mutants are ignored or unrun mutants are labeled PASS.

Learning ledger test: PASS requires recurring/systemic failure classes to create or update a ledger entry with observed failure, failure class, root cause, why prior mechanism failed, new prevention mechanism, owner files/PB IDs, regression test, and status. FAIL if repeated failures are fixed locally with no generalized ledger or anti-regression mechanism.
