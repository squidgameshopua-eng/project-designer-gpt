...
PB-47 GitHub Instruction/Knowledge Delivery Format: Instruction-copy requests require full raw `current/instructions/Instructions.md` in a copyable code block, not link-only output. Knowledge-source requests must not link the full repository ZIP as primary delivery; they require a GitHub-origin package containing only active `current/source_files` Knowledge files, or a stated GitHub limitation with artifact/release/PR-ready route before manual fallback.
