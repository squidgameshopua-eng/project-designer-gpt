...
Owner map: Instructions.md owns mandatory short triggers, user-facing language, minimal-user-action triggers, target-placement triggers, problem-class generalization triggers, compact PB-52 trigger, compact PB-53 trigger, and kernel; patch_lock_protocol.md owns change blocking; this registry owns PB IDs/status; autonomous_workflow_router.md owns route comparison, system-executable route choice, final-action route choice, route/target placement, route-level handoff contracts, problem-class routing, user-work minimization, and blocked-write routing; delegation_access_policy.md owns user-action minimization, user-facing handoff, approval/execution separation, user-only boundaries, target confirmation before user work, and target-access boundaries; delivery_protocol.md owns repository-first delivery; testing_protocol.md owns PB-48, PB-49, PB-50, PB-51, PB-52, and PB-53 tests plus gates for user-work minimization and repository-first delivery; package_state_protocol.md owns active-state truth; instruction_governance.md owns layer authority/synchronization; output_templates.md owns language/action-compression/target-result/generalized-fix/PB-52/PB-53 templates; source_safety_policy.md owns source/safety details; regression_smoke_tests.md owns concrete regression prompts; scripts/validate_package_guard.py owns mechanical PB-48/PB-49/PB-50/PB-51/PB-52/PB-53 phrase checks; scripts/build_knowledge_package.py owns the GitHub Instruction/Knowledge delivery artifact format; .github/workflows/build_knowledge_package.yml owns CI validation of that artifact.
