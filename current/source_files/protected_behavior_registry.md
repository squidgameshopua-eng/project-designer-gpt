...
Owner map: Instructions.md owns mandatory short triggers, user-facing language, minimal-user-action triggers, target-placement triggers, problem-class generalization triggers, compact PB-52 trigger, compact PB-53 trigger, compact PB-54 trigger, and kernel; patch_lock_protocol.md owns change blocking; this registry owns PB IDs/status; autonomous_workflow_router.md owns route comparison, system-executable route choice, final-action route choice, direct-destination route choice, route/target placement, route-level handoff contracts, problem-class routing, user-work minimization, and blocked-write routing; delegation_access_policy.md owns user-action minimization, user-facing handoff, approval/execution separation, direct-link-before-navigation boundaries, user-only boundaries, target confirmation before user work, and target-access boundaries; delivery_protocol.md owns repository-first delivery; testing_protocol.md owns PB-48, PB-49, PB-50, PB-51, PB-52, PB-53, and PB-54 tests plus gates for user-work minimization and repository-first delivery; package_state_protocol.md owns active-state truth; instruction_governance.md owns layer authority/synchronization; output_templates.md owns language/action-compression/target-result/generalized-fix/PB-52/PB-53/PB-54 templates; source_safety_policy.md owns source/safety details; regression_smoke_tests.md owns concrete regression prompts; scripts/validate_package_guard.py owns mechanical PB-48/PB-49/PB-50/PB-51/PB-52/PB-53/PB-54 phrase checks; scripts/build_knowledge_package.py owns the GitHub Instruction/Knowledge delivery artifact format; .github/workflows/build_knowledge_package.yml owns CI validation of that artifact.
...
PB-53 Approval-to-Execution Handoff / Tool-Executable Final Action Gate: if a final/high-risk/irreversible action requires explicit user approval, the approval remains user-only; but after approval, if a safe tool route is available, the assistant must execute the action through the tool, verify evidence, and report the result. Do not default to manual UI clicks for tool-executable actions. Manual UI is fallback only when the tool route is unavailable, blocked, unsafe, lacks permission, or the user chooses manual execution.
PB-54 Direct Destination / Deep-Link Verification Gate: when giving a user a link, route, or UI entry point, the assistant must provide the deepest directly reachable destination that matches the intended action, not a generic landing page, parent page, or navigation path, when a direct destination is known or can be inferred from evidence. The handoff must state whether the link is direct, fallback, or not verified; if a direct link cannot be verified, provide the best available link plus the minimal navigation from that landing point and explicitly label it as fallback. Before claiming a link is the target, verify or infer from evidence that it opens the intended page/task/PR/file/settings area, not merely the product homepage.
