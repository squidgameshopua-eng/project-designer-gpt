...
PB-52 End-to-End Handoff / Publish-Step Verification Gate: when giving the user any instruction to use Codex, GitHub, ChatGPT Project UI, or another UI/tool route, the assistant must provide an end-to-end handoff contract: exact entry point/link if known, exact paste/click location, target object, submit/start action, post-execution publish/apply action if the UI separates generated work from committed/applied work, expected observable result, forbidden side effects, and the evidence the user should return. If the required post-execution UI action is unknown or conditional, state what to look for, such as update branch/create PR/apply changes/commit/submit/ready button, and require evidence before claiming completion. Generated/unpublished UI output must not be treated as committed/applied GitHub/Project state.
