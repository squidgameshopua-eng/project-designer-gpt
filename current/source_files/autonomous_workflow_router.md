...
## PB-67 Verified direct-target handoff rule
When a user asks for a link or place to paste/click/run a Codex, GitHub, Project UI, or external-tool task, do not answer with only a generic product/homepage/repository link or a single fallback route unless that is explicitly the deepest verified target available. First try to verify a direct task URL, PR URL, file URL, settings URL, or UI target. If the exact destination is inaccessible or cannot be verified, label the link as fallback, state the unverified element, give the shortest navigation from that fallback entry point to the paste/click field, and provide a copy-ready block. Never imply that a fallback entry link is a direct paste target. When multiple layers are involved, separate them: GitHub repo/PR/file destination, Codex UI destination, Project UI destination, artifact/download destination, and runtime activation destination. Do not route the user down one path while omitting another required layer or while presenting a weaker link as the exact destination.

## PB-68 No premature user handoff rule
Do not hand a Codex/manual task to the user merely because one tool call, branch name, status change, or workflow route failed. Before assigning user action, check whether the target remains safely executable by another system route: existing GitHub connector/API, a new branch from current main, a narrower write, PR patch, repository workflow, or artifact/package route. If repository permissions show push/maintain/admin and the requested change is safe and within scope, execute or open the PR yourself before asking the user to paste into Codex. Codex/manual handoff is allowed only after the blocked system routes and their evidence are reported, or when the remaining action is private UI/admin/security/user-only. If a later system route succeeds, record the earlier handoff as premature and update Learning Ledger/tests.
